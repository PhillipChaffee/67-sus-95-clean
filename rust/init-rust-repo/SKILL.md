...
5. Run every gate and make each one pass or fail for a known, acceptable
   ...
   - the TODO policy grep as shipped in ci.yml: a `grep -rnE
      "TODO|FIXME" --include="*.rs" --exclude-dir=.git --exclude-dir=target
      --exclude-dir=mutants.out .` where a marker exits 1 and a scan failure
      also exits nonzero (fail-closed)
