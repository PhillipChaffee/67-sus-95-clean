...
| [`shell/`](shell/) | shellcheck: default style severity + man page's two optional checks; TODO grep | refusal: no shell type checker exists | refusal: no shell doc-comment checker exists | gate script ≥ 95% (kcov line coverage, minimum across reports) | `shfmt -d .` via `.editorconfig` | typos, gitleaks, jscpd, TODO grep, artifact linting; advisories/license refused (shell has no lockfile manifest) |
