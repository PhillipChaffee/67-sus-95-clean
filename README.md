...
| `tasks/*/notes/` | the efforts' evidence-backed decision notes: refused metric families, the go deadcode and typescript StrykerJS refusals, the gates effort's refusals and its cross-language parity audit |

## Languages

Details and reasoning live in each folder's README; the shape:

| folder | lint | types | docs | size gates | coverage | formatter | hygiene + supply-chain |
| --- | --- | --- | --- | --- | --- | --- | --- |
| [`rust/`](rust/) | clippy: pedantic + nursery + cargo + picks; rustdoc group deny; `allow_attributes_without_reason` | rustc: `missing_docs` deny | rustdoc: all 10 stable lints | `arborist` 15 (cognitive-only); ≤ 1000 effective lines | `llvm-cov` ≥ 95% on lines, regions, and functions | `rustfmt` | typos, gitleaks, jscpd, osv-scanner advisories + licenses, cargo-deny (RustSec, license policy, bans + duplicates), cargo-shear, TODO grep, artifact linting; `cargo-mutants` nightly |
| [`python/`](python/) | ruff: ALL, documented ignores; TODO marker banned | mypy `--strict` | ruff `D` (google) | `complexipy` 15 (cognitive-only); ≤ 1000 effective lines | `pytest-cov` ≥ 95% | `ruff format` | typos, gitleaks, jscpd, osv-scanner advisories + licenses, deptry, vulture, import-linter, hash-pinned lockfile, artifact linting; `mutmut` nightly |
| [`typescript/`](typescript/) | eslint: `strictTypeChecked` + jsdoc + metric caps + sonarjs + vitest plugin | tsc `strict` + 8 extras | jsdoc: require + check | sonarjs cognitive 15 (cyclomatic removed); `max-lines` ≤ 300 effective | vitest ≥ 95% ×4 | `prettier` | typos, gitleaks, jscpd, osv-scanner advisories + licenses, knip, lockfile-lint, dependency-cruiser, bidi grep, artifact linting |
| [`go/`](go/) | golangci-lint v2: strict extras (size caps, hygiene, security, test-style) | `go vet` + staticcheck | revive `exported` | `gocognit` 15 (cognitive-only; cyclop removed); ≤ 750 effective lines | gate script ≥ 95% | `gofumpt` | typos, gitleaks, jscpd, osv-scanner advisories + licenses, `go mod tidy -diff`, artifact linting |
| [`shell/`](shell/) | shellcheck: default style severity + man page's two optional checks; TODO grep | refusal: no shell type checker exists | ast-grep header-comment presence; prose doc checker refused | cognitive refused (omen probed, failed); ≤ 200 effective lines | gate script ≥ 95% (kcov line coverage, minimum across reports) | `shfmt -d .` via `.editorconfig` | typos, gitleaks, jscpd, TODO grep, artifact linting; advisories/license refused (shell has no lockfile manifest) |
...
9. **Complexity is cognitive-only, file length is effective lines, and doc
   substance is gated where mechanics exist**: every stack caps cognitive
   complexity at the Sonar S3776 default anchor of 15 (gocognit, complexipy,
   arborist, sonarjs — shell's only candidate was probed and refused, and
   cyclomatic is removed everywhere); every stack counts file length in
   effective lines, where blank and comment lines don't count, with
   thresholds 750/1000/1000/300/200 per language; and each stack carries
   the strongest mechanical doc-substance checks its tooling supports.
   Every refusal this produced is documented in
