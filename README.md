...
**code-coverage gate at ≥ 95% worldwide in every folder**. Each language folder
ships the example configuration files, a README that explains what each enforce
is for, and an installable AI skill that initializes a new repository in that
language with the whole setup.
...
2. **Every denorable or enabled-by-hand rule carries its reason** in the config
   or the README. A rule without a why gets deleted on the next review.
